...
BACKEND_LOG="${TMPDIR:-/tmp}/atlas-qe-backend.log"
FRONTEND_LOG="${TMPDIR:-/tmp}/atlas-qe-frontend.log"
...
for port in 8300 5300; do
  if lsof -nP -iTCP:"$port" -sTCP:LISTEN >/dev/null 2>&1; then
    fail "Porta $port já está ocupada; o processo existente foi preservado."
  fi
done

echo "🔐 Atlas Queryable Encryption"
echo "============================="

# Backend
echo "▶ Iniciando backend (porta 8300)..."
cd "$BASE/backend"
UVICORN_ARGS=(main:app --host 127.0.0.1 --port 8300)
[[ "${POV_DEV:-0}" == "1" ]] && UVICORN_ARGS+=(--reload)
venv/bin/uvicorn "${UVICORN_ARGS[@]}" > "$BACKEND_LOG" 2>&1 &
BACKEND_PID=$!
echo "  PID: $BACKEND_PID"

if ! wait_for_url "http://127.0.0.1:8300/health/live" 30; then
...
# Frontend
echo "▶ Iniciando frontend (porta 5300)..."
cd "$BASE/frontend"
if [[ "${POV_DEV:-0}" != "1" ]] && {
  [[ ! -f dist/index.html ]] ||
...
if [[ "${POV_DEV:-0}" == "1" ]]; then
  FRONTEND_CMD=(node_modules/.bin/vite --host 127.0.0.1 --port 5300 --strictPort)
else
  FRONTEND_CMD=(node_modules/.bin/vite preview --host 127.0.0.1 --port 5300 --strictPort)
fi
"${FRONTEND_CMD[@]}" > "$FRONTEND_LOG" 2>&1 &
FRONTEND_PID=$!
echo "  PID: $FRONTEND_PID"

if ! wait_for_url "http://127.0.0.1:5300" 30; then
...
echo "Preflight:  http://localhost:8300/preflight"
PREFLIGHT_STATUS="$(curl --silent --output /dev/null --write-out '%{http_code}' --max-time 5 \
  "http://127.0.0.1:8300/preflight" || true)"
